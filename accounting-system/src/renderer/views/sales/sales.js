...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Promise.all([
        ...
        // Debug: log select computed colors after data is ready
        logCustomerSelectColors();
    });
});
    ...
    app.innerHTML = `
        <nav class="top-nav">
            ...
            <ul class="nav-links">
                ...
                <li><a href="#" class="active">المبيعات</a></li>
        ...
        <div class="page-header">
            <div class="page-title">فواتير المبيعات</div>
        </div>

        <!-- New Invoice Form Section -->
        <div id="invoiceForm" class="invoice-form-container" style="display: block;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2 style="margin: 0; color: var(--primary-color);">تسجيل فاتورة بيع جديدة</h2>
            </div>
            ...
            <div class="items-section">
                ...
                <button class="btn btn-outline" onclick="addInvoiceRow()">+ إضافة صنف آخر</button>
            </div>

            <div class="invoice-footer-grid">
                <div class="notes-section">
                    <div class="form-group">
                        <label>ملاحظات / شروط الدفع</label>
                        <textarea id="invoiceNotes" class="form-control" rows="4" placeholder="أدخل أي ملاحظات إضافية هنا..."></textarea>
                    </div>
                </div>
...
function initializeElements() {
    ...
    customerSelect.addEventListener('change', () => {
        if (customerSelect.value && invoiceItemsBody.children.length === 0) {
            addInvoiceRow();
        }
    });
}
...
async function loadInvoiceForEdit(id) {
    ...
    try {
        ...
        // Update UI
        document.querySelector('#invoiceForm h2').textContent = 'تعديل فاتورة بيع';
...
function addInvoiceRow(existingItem = null) {
    ...
    allItems.forEach(item => {
        ...
        itemsOptions += `<option value="${item.id}" data-price="${item.sale_price}" ${isSelected}>${item.name} (متاح: ${item.stock_quantity})</option>`;
    });
...
function logCustomerSelectColors() {
    const select = document.getElementById('customerSelect');
    if (!select) return;
    const styles = getComputedStyle(select);
    console.log('[customerSelect] theme:', document.body.dataset.theme || document.documentElement.dataset.theme || 'unknown', 'bg:', styles.backgroundColor, 'color:', styles.color);
}
...
async function updateInvoice() {
    if (!editingInvoiceId) {
        alert('لا يمكن تحديث الفاتورة: رقم تعريف الفاتورة غير موجود');
        return;
    }
    ...
    if (!isValid || items.length === 0) {
        alert('الرجاء إدخال جميع بيانات الأصناف بشكل صحيح');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            window.location.href = 'index.html'; // Or reload/clear form
        } else {
...
async function saveInvoice() {
    ...
    if (!isValid) {
        alert('الرجاء التأكد من إدخال الأصناف والكميات بشكل صحيح');
        return;
    }
...
async function resetForm() {
    ...
    document.querySelector('#invoiceForm h2').textContent = 'تسجيل فاتورة بيع جديدة';
    const saveBtn = document.querySelector('#invoiceForm .btn-success');
    saveBtn.textContent = 'حفظ وترحيل الفاتورة';
