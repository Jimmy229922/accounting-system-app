...
function initializeElements() {
    ...
    if (salesReturnsState.dom.printPreviewModal) {
        salesReturnsState.dom.printPreviewModal.addEventListener('click', (event) => {
            if (event.target === salesReturnsState.dom.printPreviewModal) {
                closePrintPreview();
            }
        });
    }
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            if (salesReturnsState.dom.printPreviewModal?.classList.contains('is-open')) {
                closePrintPreview();
            }
        }
    });
...
async function setPrintPreviewOpen(open) {
    const modal = salesReturnsState.dom.printPreviewModal;
    if (!modal) return;
    if (open) {
...
async function printReturn() {
    const customerSelect = salesReturnsState.dom.customerSelect;
    const customerName = customerSelect.options[customerSelect.selectedIndex]?.text || '';
    const returnNumber = salesReturnsState.dom.returnNumberInput.value;
    const returnDate = salesReturnsState.dom.returnDateInput.value || new Date().toLocaleDateString('ar-EG');
    ...
    const itemNamesMap = {};
    items.forEach(item => {
        const itemObj = salesReturnsState.currentInvoiceItems.find(i => i.item_id === item.item_id);
        itemNamesMap[item.item_id] = itemObj ? (itemObj.item_name || itemObj.name || '') : '';
    });

    const totalAmount = parseFloat(salesReturnsState.dom.returnTotal?.textContent || '0') || 0;

    // 2. Submit the return if not locked
    const wasEditing = !!salesReturnsState.editingReturnId;
