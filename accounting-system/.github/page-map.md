...
│   ├── main.js                    ← نقطة الدخول، إدارة النوافذ
│   ├── preload.js                 ← جسر electronAPI (85 قناة IPC)
│   ├── db.js                      ← إنشاء الجداول والـ migrations
│   ├── ipcHandlers.js             ← تسجيل كل الـ handlers
│   ├── inviteConfig.js            ← إعدادات كود الدعوة
│   └── handlers/                  ← ملفات الـ IPC handlers (18 ملف)
│
└── renderer/                      ← واجهة المستخدم
    ├── views/                     ← صفحات التطبيق (19 صفحة)
    ├── assets/
    │   ├── js/                    ← سكربتات مشتركة
...
- **Forms**: `.invoice-form-container`, `.invoice-shell`, `.form-title-row`, `.invoice-top-grid`, `.form-group`, `.form-control`
- **Tables**: `.items-section`, `.items-section-head`, `.items-table-wrap`, `.items-table`
- **Footer/Totals**: `.invoice-footer-grid`, `.notes-section`, `.totals-panel`, `.total-row`, `.grand-total`
- **Buttons**: `.btn-success`, `.btn-outline`
- **Other**: `.customer-balance`, `.balance-positive`, `.balance-negative`, `.unit-label`, `.row-total`, `.remove-row`
...
**CSS Classes الرئيسية**:
- **Layout**: `.content`, `.page-header`, `.page-title`
- **Forms**: `.invoice-form-container`, `.invoice-top-grid`, `.form-group`, `.form-control`
- **Tables**: `.table-card`, `.table`, `.items-table`
- **Footer/Totals**: `.invoice-footer-grid`, `.totals-panel`, `.total-row`, `.grand-total`
- **Buttons**: `.btn-primary`, `.btn-success`, `.btn-danger`, `.btn-view`
- **Badges**: `.status-badge`
- **Other**: `.autocomplete-items`, `.autocomplete-active`

**Dark Mode CSS**: overrides لـ `.table tr:hover`, `select.form-control`
...
**قنوات IPC**: `get-auth-users`, `create-auth-user`, `set-auth-user-active`, `reset-auth-user-password`, `get-active-auth-user`, `get-auth-session-token`
**جداول DB**: `auth_users`, `auth_sessions`
**CSS Classes الرئيسية**:
...
- **Buttons**: `.btn-secondary`, `.btn-warning`, `.btn-sm`
...
**قنوات IPC**: `get-invite-status`, `submit-invite-code`
**جداول DB**: `settings`
**CSS Classes الرئيسية**:
- **Layout**: `.invite-card`
- **Forms**: `.form-group`, `.btn-primary`
...
## قنوات IPC الكاملة (85 قناة)

### auth.js (10)
`get-auth-status` · `setup-auth-account` · `login-auth-account` · `get-active-auth-user` · `get-auth-users` · `create-auth-user` · `set-auth-user-active` · `reset-auth-user-password` · `get-invite-status` · `submit-invite-code`
...
### purchaseReturns.js (4)
`get-purchase-returns` · `get-supplier-purchase-invoices` · `save-purchase-return` · `delete-purchase-return`
...
### salesReturns.js (4)
`get-sales-returns` · `get-customer-sales-invoices` · `save-sales-return` · `delete-sales-return`
